...
public class YouTubeService {
  ...
  public SearchListResponse search(String query) throws IOException {
    if (currentKeyIndex.get() >= apiKeys.size()) {
      System.err.println(
        "ROTAÇÃO DE CHAVES: Todas as chaves do YouTube atingiram a cota."
      );
      throw new IOException(
        "Todas as chaves da API do YouTube atingiram a cota."
      );
    }
    ...
    try {
      YouTube.Search.List searchRequest = youtube
        .search()
      ...
      System.out.println(
        "LOG COTA: Busca usou 100 unidades (Chave " +
        currentKeyIndex.get() +
        "). Total: " +
        quotaUsedToday.get()
      );
      ...
      boolean isQuotaError = e
        .getDetails()
        .getErrors()
        .stream()
        .anyMatch(err ->
          "quotaExceeded".equals(err.getReason()) ||
          "rateLimitExceeded".equals(err.getReason())
        );

      if (
        (e.getStatusCode() == 403 || e.getStatusCode() == 429) && isQuotaError
      ) {
        int failedIndex = currentKeyIndex.get();
        System.err.println(
          "AVISO: Cota da Chave " +
          failedIndex +
          " excedida (Status " +
        ...
        if (currentKeyIndex.compareAndSet(failedIndex, failedIndex + 1)) {
          System.err.println(
            "ROTAÇÃO: Mudando para a próxima chave (Índice " +
            (failedIndex + 1) +
            ")."
          );
        }
  ...
  public VideoListResponse getVideoDetails(String videoId) throws IOException {
    ...
    try {
      YouTube.Videos.List videoRequest = youtube
        .videos()
        .list(Collections.singletonList("snippet"));
      ...
      System.out.println(
        "LOG COTA: Details usou 1 unidade (Chave " +
        currentKeyIndex.get() +
        "). Total: " +
        quotaUsedToday.get()
      );
      ...
      if (
        ...
        System.err.println(
          "AVISO: Cota da Chave " +
          currentKeyIndex.get() +
          " excedida. A rodar para a próxima chave."
        );
  ...
  public void resetQuotaCounter() {
    System.out.println(
      "SCHEDULER: A resetar contadores. Cota antiga: " + quotaUsedToday.get()
    );
